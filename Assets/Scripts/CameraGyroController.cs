...
public class CameraGyroController : MonoBehaviour
{
    ...
    private void Update()
    {
        ApplyGyroRotation();
        ApplyCalibration();
        _rawGyroRotation.rotation = new Quaternion(Mathf.Clamp(_rawGyroRotation.rotation.x , -0.3f,0.3f),0, 0, 0);
       // Debug.Log(" x "+ camera.transform.rotation.x + " y " + camera.transform.rotation.y + " w " + camera.transform.rotation.w+ " z " + camera.transform.rotation.z);
